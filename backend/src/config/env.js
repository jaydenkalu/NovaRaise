...
function validateEnv() {
  ...
  if (missing.length) {
    ...
    process.stderr.write(
      `\n[novaraise] Cannot start: missing required environment variables:\n${list}\n\nSet them in your .env file.\n\n`
    );
    process.exit(1);
  }
  ...
  if (process.env.API_KEY_PEPPER === process.env.JWT_SECRET) {
    process.stderr.write(
      '\n[novaraise] Cannot start: API_KEY_PEPPER must differ from JWT_SECRET — secrets must not be reused across concerns.\n\n'
    );
    process.exit(1);
  }
  ...
  if (storageConfigured && storageMissing.length) {
    ...
    process.stderr.write(
      `\n[novaraise] Cannot start: incomplete storage configuration. Set all of:\n${STORAGE_VARS.join(', ')}\n\nMissing:\n${list}\n\n`
    );
    process.exit(1);
  }
  ...
  try {
    ...
    process.stderr.write(`\n[novaraise] Cannot start: ${err.message}\n\n`);
    process.exit(1);
  }
  ...
  if (!process.env.PLATFORM_APPROVER_USER_ID) {
    process.stderr.write(
      '[novaraise] Warning: PLATFORM_APPROVER_USER_ID not set — withdrawal approvals are open to all users (dev mode)\n'
    );
  }
  if (!process.env.JWT_EXPIRES_IN) {
    process.stderr.write(
      '[novaraise] Warning: JWT_EXPIRES_IN not set — access tokens will use the default expiry (15m)\n'
    );
  }
}
