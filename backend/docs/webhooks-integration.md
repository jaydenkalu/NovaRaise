...
NovaRaise can `POST` JSON payloads to your HTTPS URL when lifecycle events occur. Configure endpoints and signing secrets under **Developer** in the app, or via `POST /api/webhooks` (JWT or API key with `developer` scope).
...
- **Headers:**
  ...
  - `X-NovaRaise-Event`: event name (e.g. `contribution.received`)
  - `X-NovaRaise-Delivery-Id`: unique delivery UUID (for idempotency on your side)
  - `X-NovaRaise-Signature`: `sha256=<hex>` where `<hex>` is **HMAC-SHA256** of the **raw request body bytes** using your endpoint’s **webhook secret** (shown once when the endpoint is created).
...
function verifyNovaRaiseWebhook(rawBodyBuffer, signatureHeader, secret) {
...
Use the **raw** body string (before JSON parsing) so the signature matches what NovaRaise signed.

## API keys (server-to-server)

Create keys with `POST /api/api-keys`. Send `Authorization: Bearer cp_live_<secret>` on NovaRaise API requests.
